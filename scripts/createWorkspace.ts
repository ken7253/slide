...
const crateWorkspace = (name: string | null) => {
  const workspaceName = name ?? dayjs(new Date()).format('YYYY-MM-DD').toString();
  ...
  exec(`npm init -w ${workspaceName} -y`)
    ...
    .then(() => {
      ...
      // スライドファイルの作成
      const templateFilePath = path.join(process.cwd(), 'scripts', 'assets', 'slides.md');
  ...
  .prompt([
    {
      ...
      validate: (input) => (reservedDirChars.test(input) ? 'ディレクトリ名に指定できない文字列が含まれています' : true),
