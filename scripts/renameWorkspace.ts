...
const hasWorkspaces = (
  packageJSON: Object
): packageJSON is { workspaces: string[] } => {
  const hasProperty = Object.hasOwn(packageJSON, "workspaces");
  if (!hasProperty) return false;
  const isArray = Array.isArray(
    (packageJSON as { workspaces: unknown }).workspaces
  );
  return isArray;
};
    ...
    inquirer
      .prompt([
        ...
        },
        {
          ...
          validate: (input) => {
            return reservedDirChars.test(input)
              ? "The string entered cannot be used as a directory name."
              : true;
          },
        },
      ])
      .then((answer) => {
        ...
        const hasAnswer = (
          v: unknown
        ): v is { currentName: unknown; newName: unknown } => {
